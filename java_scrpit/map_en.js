...
function initLeafletMap() {
  ...
  centerIndicator.innerHTML = `
    <div class="pin"></div>
    <span id="deliverToText">Move the map to choose location...</span>
  `;
...
async function getLocationFromMap() {
  ...
  try {
    ...
    if (!address || address.country_code !== "eg") {
      document.getElementById("deliverToText").textContent = "❌ Outside Egypt";
    ...
    document.getElementById(
      "deliverToText"
    ).textContent = `📍 Deliver to: ${finalName}`;
  } catch {
    document.getElementById("deliverToText").textContent =
      "⚠️ Error detecting location";
  }
}
...
confirmLocationBtn.addEventListener("click", () => {
  if (!selectedLocation) {
    Swal.fire(
      "⚠️",
      "Please move the map to select a valid location",
      "warning"
    );
    return;
  }
...
function performSearch() {
  ...
  if (selectedLocation) {
    ...
    Swal.fire("⚠️", "Please select your location first", "warning");
  }
}
...
async function showSuggestions(query) {
  ...
  const res = await fetch(
    `https://nominatim.openstreetmap.org/search?q=${query}&countrycodes=eg&format=json&addressdetails=1&limit=10&accept-language=en`
  );
